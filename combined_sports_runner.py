...
def main():
    ...
    if LIVE_LEARNING_MODE:
        logger.info("="*80)
        logger.info("🔬 LIVE LEARNING MODE - STABILITY & VERIFICATION")
        logger.info("="*80)
        logger.info("📊 Stability Mode Controls (Dec 25, 2025):")
        logger.info("   • EV deflator: 0.4x applied globally")
        logger.info("   • Hard EV cap: 25% maximum")
        logger.info("   • Blocked EV bands: 50-100% (structurally -EV)")
        logger.info("   • SGP: DISABLED")
        logger.info("   • CORNERS: 30% portfolio cap")
        logger.info("   • CLV tracking: ENABLED (verify edge, not P/L)")
        logger.info("   • Unit-based flat staking (no Kelly scaling)")
