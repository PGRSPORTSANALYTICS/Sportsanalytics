...
try:
    ...
    logger.error(
        "Could not import db_connection — publisher cannot run without DB")
    raise
...
def discover_leagues(picks: List[dict]) -> List[str]:
    ...
    for lg in leagues:
        mapped = "MAPPED" if lg in LEAGUE_WEBHOOKS and LEAGUE_WEBHOOKS[
            lg] else "DEFAULT"
...
def _format_kickoff(match_date) -> str:
    ...
    try:
        ...
        return dt.strftime("%b %d, %H:%M UTC"
                           ) if dt.hour or dt.minute else dt.strftime("%b %d")
    except Exception:
        return str(match_date)
...
def format_message(pick: dict) -> dict:
    ...
    description = (f"**Match:** {home} vs {away}\n"
                   f"**League:** {league}\n"
                   f"**Market:** {selection}\n"
                   f"**Odds:** {odds:.2f}\n"
                   f"**EV:** {edge:.1f}%\n"
    ...
    embed = {
        ...
        "color": 3066993,
        "footer": {
            "text": f"PGR Sports Analytics — {league} | Analysis Only"
        },
...
def run_publish_cycle():
    ...
    for pick in picks:
        ...
        if not webhook_url:
            logger.warning(
                f"No webhook available for league '{league}' — skipping")
            stats["errors"] += 1
            continue
        ...
        if success:
            ...
            pick_id = pick.get("id", "?")
            logger.info(
                f"Posted pick #{pick_id}: {pick.get('home_team')} vs {pick.get('away_team')} [{league}]"
            )
        else:
            stats["errors"] += 1
...
def main():
    ...
    logger.info(
        f"Poll interval: {POLL_INTERVAL}s | Dedupe TTL: {DEDUPE_TTL_HOURS}h | Rate limit: {RATE_LIMIT_SECONDS}s/webhook"
    )

    mapped_count = sum(1 for v in LEAGUE_WEBHOOKS.values() if v)
    logger.info(
        f"League webhooks configured: {mapped_count} | Default webhook: {'SET' if DEFAULT_WEBHOOK else 'MISSING'}"
    )
