...
class RealFootballChampion:
    ...
    def _generate_analysis_text(self, analysis: Dict, predicted_score: str, home_team: str, away_team: str) -> str:
        """Generate human-readable analysis text from enriched analysis data"""
        analysis_parts = []
        
        analysis_parts.append(f"📊 **WHY {predicted_score}?**\n")
        ...
        if home_xg > 0 or away_xg > 0:
            analysis_parts.append(f"⚽ **Expected Goals**: {home_team} {home_xg:.1f} xG, {away_team} {away_xg:.1f} xG")
            total_xg = home_xg + away_xg
            if total_xg < 2.0:
                analysis_parts.append("Low-scoring match expected")
            elif total_xg > 3.0:
                analysis_parts.append("High-scoring match expected")
        
        home_form = analysis.get('home_form', {})
        away_form = analysis.get('away_form', {})
        
        if home_form.get('matches_played', 0) > 0:
            home_wr = home_form.get('win_rate', 0)
            home_gpg = home_form.get('goals_per_game', 0)
            home_cpg = home_form.get('conceded_per_game', 0)
            analysis_parts.append(f"🏠 **{home_team} Form**: {home_wr:.0f}% wins, {home_gpg:.1f} goals/game, {home_cpg:.1f} conceded/game")
        
        if away_form.get('matches_played', 0) > 0:
            away_wr = away_form.get('win_rate', 0)
            away_gpg = away_form.get('goals_per_game', 0)
            away_cpg = away_form.get('conceded_per_game', 0)
            analysis_parts.append(f"✈️ **{away_team} Form**: {away_wr:.0f}% wins, {away_gpg:.1f} goals/game, {away_cpg:.1f} conceded/game")
        
        h2h = analysis.get('h2h', {})
        if h2h.get('matches_played', 0) >= 3:
            h2h_matches = h2h.get('matches_played', 0)
            h2h_avg_goals = h2h.get('avg_total_goals', 0)
            h2h_home_goals = h2h.get('avg_team1_goals', 0)
            h2h_away_goals = h2h.get('avg_team2_goals', 0)
            analysis_parts.append(f"📜 **Head-to-Head** ({h2h_matches} games): Avg {h2h_avg_goals:.1f} total goals ({h2h_home_goals:.1f}-{h2h_away_goals:.1f})")
        
        home_standings = analysis.get('home_standings', {})
        away_standings = analysis.get('away_standings', {})
        if home_standings.get('rank') and away_standings.get('rank'):
            analysis_parts.append(f"📊 **League Position**: {home_team} #{home_standings['rank']}, {away_team} #{away_standings['rank']}")
        
        odds_movement = analysis.get('odds_movement', {})
        if odds_movement.get('movement_percent', 0) != 0:
            movement = odds_movement['movement_percent']
            if abs(movement) > 5:
                direction = "dropped" if movement < 0 else "risen"
                analysis_parts.append(f"📈 **Odds Movement**: {abs(movement):.1f}% {direction}")
                if odds_movement.get('sharp_money_indicator'):
                    analysis_parts.append("💰 Sharp money detected (professional bettors)")
        
        lineups = analysis.get('lineups', {})
        if lineups.get('lineups_confirmed'):
            analysis_parts.append("✅ **Lineups Confirmed**")
        injuries = lineups.get('home_injuries', 0) + lineups.get('away_injuries', 0)
        if injuries > 0:
            analysis_parts.append(f"⚠️ {injuries} key injuries reported")
        
        quality = analysis.get('quality_score', 0)
        if quality >= 70:
            analysis_parts.append(f"⭐ **High Quality**: {quality:.0f}/100 rating")
