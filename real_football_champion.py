    ...
    def run_exact_score_analysis(self):
        ...
        for match_data in selected_matches:
            ...
            if best_score and best_probability > 0.02:  # At least 2% probability (more realistic for exact scores)
                ...
                passes_odds = 8 <= final_odds <= 12  # AI-learned: 8-12x profitable, 12x+ loses money (Dec 7 backtest)
    ...
    def _save_data_picks(self, data_picks: list) -> int:
        ...
        for pick in data_picks:
            try:
                ...
                print(f"⚠️ Failed to save data pick: {e}")
        return saved
...
def run_single_cycle():
    ...
    try:
        ...
        try:
            value_engine = ValueSinglesEngine(champion, ev_threshold=0.0, min_confidence=50)
            ...
            data_picks = getattr(value_engine, '_data_picks', [])
            if data_picks:
        ...
        if summer_cap_reached:
            ...
            try:
                summer_engine = ValueSinglesEngine(champion, ev_threshold=0.0, min_confidence=50)
