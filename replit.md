...
- **Basketball Settings (Dec 6, 2025):** Widened max odds from 5.50 to 12.00. Settings: **30% confidence** (balanced - allows moderate underdogs), 15 singles max, odds range 1.40-12.00, 3 parlays max, 3% EV minimum edge. Parlays capped at 50x max. Analysis showed: 50%+ favorites = +43 units, 15-30% underdogs = +14.8 units, 5-15% longshots = +1.4 units (break-even). 30% threshold captures profitable moderate underdogs while avoiding risky longshots.
...
- **AI-Learned SGP Odds Filter (Dec 3, 2025 - 4 DAY TRIAL):** First automated AI learning from 653 settled bets. Analysis showed: Odds 4-6 range has +12.8% edge vs Odds 2-4 with -14.8% (losing money). SGP MIN_ODDS raised from 2.5 to 4.0, MAX_ODDS lowered from 10.0 to 6.0. Trial runs Dec 3-7, 2025. Previous settings: MIN_ODDS=2.5, MAX_ODDS=10.0.
- **Realistic SGP Margin Calibration (Dec 5, 2025):** Updated SGP pricing from 7% margin to 40% margin based on real-world Bet365 feedback. User reported Bet365 offering 3.20 odds vs system estimate of 5.59 for same SGP. Investigation showed real bookmakers apply 40-54% margins on SGP builders. This ensures predicted odds match reality and prevents users from chasing bets unavailable at displayed prices.
- **Discord Result Notifications (Dec 5, 2025):** Integrated Discord webhooks for automated result posting across all prediction types. Each product sends win/loss notifications to its dedicated channel (Final Score, SGP, ML Parlays, Value Singles, College Basketball). Notifications include match result, pick, odds, and profit/loss in both USD and SEK.
- **PGR Final Score Strategy v1.0 (Dec 7, 2025):** Data-driven odds filter based on 217 settled bets analysis. Key findings: 10-12x odds range has 13.3% hit rate vs 8.9% for 12-15x range. Settings updated: (1) Odds narrowed from 7-16x to **10-12x sweet spot**, (2) EV minimum lowered to 5%, (3) Proven scores: 2-1 (10.8% hit rate, 12 wins), 2-0 (8.1%), 0-1 (20%), 1-1, 1-0, (4) Confidence ≥70%. Basketball duplicate fix: Added pre-insert check to prevent same match/selection being saved multiple times per day.
