...
- **Basketball Settings (Dec 6, 2025):** Widened max odds from 5.50 to 12.00 to capture high-EV underdog picks that were being filtered out. Settings: 50% confidence, 15 singles max, odds range 1.40-12.00, 3 parlays max, 3% EV minimum edge. Parlays capped at 50x max odds to avoid unrealistic lottery tickets (was allowing 3000+ odds before). **Critical fix:** Lowered confidence threshold from 50% to 5% - the old 50% filter was mathematically impossible for underdogs (odds 5+ means <20% probability by definition).
