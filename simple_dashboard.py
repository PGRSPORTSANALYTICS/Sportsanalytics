...
# ============================================================================
# SUPPLEMENTARY TRACKER: BTTS & ML BETS
# ============================================================================
...
try:
    ...
    SELECT 
        ...
        COUNT(CASE WHEN outcome IN ('win', 'won') THEN 1 END) as wins,
        COUNT(CASE WHEN outcome IN ('loss', 'lost') THEN 1 END) as losses,
        SUM(CASE WHEN outcome IS NOT NULL AND outcome != '' AND outcome NOT IN ('unknown', 'void') THEN profit_loss ELSE 0 END) as profit,
    ...
    LIMIT 20
    """
    settled_bets = pd.read_sql_query(settled_query, conn)
    ...
    with col1:
        st.markdown("### ⚽ BTTS Performance")
        if not btts_stats.empty and btts_stats.iloc[0]['total'] > 0:
            stats = btts_stats.iloc[0]
            ...
            with metric_col1:
                st.metric("Bets", int(stats['total']))
            with metric_col2:
                st.metric("Win Rate", f"{win_rate:.1f}%")
            with metric_col3:
                profit_color = "normal" if stats['profit'] >= 0 else "inverse"
                st.metric("P&L", f"{stats['profit']:,.0f} SEK", delta=f"ROI: {roi:+.1f}%")
        else:
            st.info("📊 No BTTS bets tracked yet")
    
    with col2:
        st.markdown("### 🏆 Moneyline Performance")
        if not ml_stats.empty and ml_stats.iloc[0]['total'] > 0:
            stats = ml_stats.iloc[0]
            settled = stats['wins'] + stats['losses']
            win_rate = (stats['wins'] / settled * 100) if settled > 0 else 0
            roi = (stats['profit'] / stats['staked'] * 100) if stats['staked'] > 0 else 0
            ...
            st.info("📊 No ML bets tracked yet")
    ...
    if not active_bets.empty:
        ...
        for _, bet in active_bets.iterrows():
            market_icon = "⚽" if bet['market'] == 'btts' else "🏆"
            market_label = "BTTS" if bet['market'] == 'btts' else "ML"
            
            col1, col2, col3 = st.columns([3, 1, 1])
            with col1:
                st.markdown(f"{market_icon} **{bet['home_team']} vs {bet['away_team']}**")
                st.caption(f"{market_label}: {bet['selection']}")
            with col2:
                st.markdown(f"**{bet['odds']:.2f}x**")
            with col3:
                st.caption(f"📅 {bet['match_date']}")
        
        st.markdown("")
    
    # Recent settled bets
    if not settled_bets.empty:
        with st.expander(f"📊 Last {len(settled_bets)} Settled Bets", expanded=False):
            display_settled = settled_bets.copy()
            display_settled['Match'] = display_settled['home_team'] + ' vs ' + display_settled['away_team']
            display_settled['Market'] = display_settled['market'].apply(lambda x: 'BTTS' if x == 'btts' else 'ML')
            display_settled['P&L'] = display_settled['profit_loss'].apply(lambda x: f"{x:,.0f} SEK")
            
            table = display_settled[['Match', 'Market', 'selection', 'odds', 'result', 'P&L']].copy()
            table.columns = ['Match', 'Type', 'Selection', 'Odds', 'Result', 'P&L']
            ...
            styled = table.style.apply(color_rows, axis=1)
            st.dataframe(styled, use_container_width=True, height=400)

except Exception as e:
    st.warning(f"⚠️ BTTS/ML tracker not yet initialized. Add your first bet to start tracking!")
